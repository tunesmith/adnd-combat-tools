...
export const monsterLevelMessages = (options: {
  ...
  if (lvl === MonsterLevel.One) {
    ...
    } else if (lvl === MonsterLevel.Seven) {
      messages.push({
        kind: 'table-preview',
        id: 'monsterSeven',
        title: 'Monster (Level 7)',
        sides: monsterSeven.sides,
        entries: monsterSeven.entries.map((e) => ({
          range: rangeText(e.range),
          label: MonsterSeven[e.command] ?? String(e.command),
        })),
        context: { kind: 'wandering', level: dungeonLevel },
      });
    } else if (lvl === MonsterLevel.Eight) {
      messages.push({
        kind: 'table-preview',
        id: 'monsterEight',
        title: 'Monster (Level 8)',
        sides: monsterEight.sides,
        entries: monsterEight.entries.map((e) => ({
          range: rangeText(e.range),
          label: MonsterEight[e.command] ?? String(e.command),
        })),
        context: { kind: 'wandering', level: dungeonLevel },
      });
    } else if (lvl === MonsterLevel.Nine) {
      messages.push({
        kind: 'table-preview',
        id: 'monsterNine',
