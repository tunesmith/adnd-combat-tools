...
import {
  ...
  getInitiativeGraphNodeStatusLabel,
...
import { InitiativeGraphInspectorPanel } from './InitiativeGraphInspectorPanel';
...
const GRAPH_NODE_FILL_BY_SIDE: Record<
  ...
  Record<'pending' | InitiativeGraphNodeStatus, string>
> = {
  party: {
    pending: '#6F8E34',
    resolved: '#556C2E',
...
const getGraphNodeFill = (
  side: InitiativePlaytestSide,
  status: InitiativeGraphNodeStatus | undefined
...
const InitiativePlayground = ({
  ...
  const enabledGraphNodeIds = useMemo(
    () => buildInitiativeGraphEnabledNodeIds(attackGraph, graphNodeStatusById),
    [attackGraph, graphNodeStatusById]
  );
  const selectedGraphInspectorModel = useMemo(
    () =>
      selectedGraphNodeId
        ? buildInitiativeGraphInspectorModel(
            resolvedRound,
        ...
        : undefined,
    [graphNodeStatusById, resolvedRound, selectedGraphNodeId]
  );
            ...
            <div className={styles['graphWorkspace']}>
              <div className={styles['graphViewportShell']}>
                <div ref={graphViewportRef} className={styles['graphViewport']}>
                        ...
                        {graphLayout.nodes.map((layoutNode) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {nodeStatus ? (
                                <g
                                  aria-label={getInitiativeGraphNodeStatusLabel(
                                    nodeStatus
                                  )}
                                >
                                  <circle
                  ...
                  {selectedGraphInspectorModel &&
                  selectedGraphPopoverPosition ? (
                    <InitiativeGraphInspectorPanel
                      ref={graphPopoverRef}
                      className={styles['graphPopover']}
                      model={selectedGraphInspectorModel}
                      style={selectedGraphPopoverPosition}
                      onClick={(event) => event.stopPropagation()}
                      onOpenNode={(nodeId) => openGraphNode(nodeId, true)}
                      onResolve={(nodeId) =>
                        applyGraphNodeStatus(nodeId, 'resolved')
                      }
                      onMarkLost={(nodeId) =>
                        applyGraphNodeStatus(nodeId, 'lost')
                      }
                      onClearStatus={(nodeId) =>
                        applyGraphNodeStatus(nodeId, undefined)
                      }
                    />
                  ) : null}
                </div>
